    ...
    while True:
        ...
        if "!run" in last_line.lower():
            ...
            if "whatsapp от дениса:" in last_line.lower():
                role = (
                    "Денис написал тебе лично в WhatsApp. Ответь ему прямо, тепло, профессионально и по делу. "
                    "Твой ответ обязательно ДОЛЖЕН начинаться строго с '[WhatsApp Reply]: '. "
                    "Ты можешь использовать инструменты !google, !погода или !run, если Денис просит об этом."
                )
                ...
                role = (
                    ...
                    "Если это сообщение важное и требует внимания Дениса (например, важные новости о его сыне Антоне, "
                    "вопросы по его работе электриком или планах), ты должен составить для Дениса краткое проактивное уведомление. "
                    "Это уведомление ДОЛЖНО начинаться строго с '[WhatsApp Reply]: [ИИ Уведомление]: ' и будет "
                    "отправлено Денису на телефон. Если сообщение не требует внимания, ответь кратко в чат-рум без префикса."
                )
                prompt = f"Ты ИИ Дениса. {role}\n\nИстория переписки в WhatsApp для контекста:\n{wa_context}\n\nТекущий чат-рум:\n{context}"
            else:
